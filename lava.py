...
def update_devices():
    ...
    #cfg["dpad"].addstr(y, 0, "Devices (refresh %d)" % (now - cache["device"]["time"]))
    di = 0
    for device in dlist:
        if di > 20:
            di += 1
    ...
    cfg["dpad"].addstr(0, 0, "Devices %d/%d (refresh %d/%d)" %
        (
        cfg["devices"]["count"],
        cfg["devices"]["max"],
...
def main(stdscr):
    ...
    while not exit:
        ...
        if cfg["devices"]["enable"]:
            update_devices()
            cfg["dpad"].refresh(0, 0, y, 0, rows - 1, cols - 1)
            y += cfg["devices"]["count"] + 1
